...
public final class RSAUtil {
    ...
    public static String parsingRSAString(String data, String keyStr, Integer keyType, Integer model) throws IllegalAccessException {
        try{
            if(keyType==0&&model==0){
                //私钥解密
                RSAPrivateKey privateKey=(RSAPrivateKey) RSAUtil.getPrivateKey(keyStr);
    ...
    public static void main(String... args) throws Exception {
        ...
        //key
        PublicKey publicKey2 = RSAUtil.getPublicKey(publicKeyData2Save);
        PrivateKey privateKey2 = RSAUtil.getPrivateKey(privateKeyData2Save);

        LogUtil.msg("public key: \n" + publicKey.toString());
        LogUtil.msg("public keyData: \n" + Arrays.toString(publicKeyData));
        LogUtil.msg("public save keyStr: \n" + publicKeyStrSave);
        LogUtil.msg("public save keyData: \n" + Arrays.toString(publicKeyData2Save));

        LogUtil.msg("private key: \n" + privateKey.toString());
        LogUtil.msg("private keyData: \n" + privateKeyData);
        LogUtil.msg("private save keyStr: \n" + privateKeyStrSave);
        LogUtil.msg("private save keyData: \n" + Arrays.toString(privateKeyData2Save));

        LogUtil.msg("还原后的公钥: \n" + publicKey2.toString());
        LogUtil.msg("还原后的私钥: \n" + privateKey2.toString());
        /******************* keyStr->keyData->key ********************/
        LogUtil.msg("by decoder.decode\n");
        LogUtil.msg("public keyDataSave 还原：" + (Arrays.equals(publicKeyData, publicKeyData2Save)));
        LogUtil.msg("private keyDataSave 还原：" + (Arrays.equals(privateKeyData, privateKeyData2Save)));
        LogUtil.msg("public key 还原：" + (publicKey.equals(publicKey2)));
        LogUtil.msg("private key 还原：" + (privateKey.equals(privateKey2)));
    ...
    public static void Test(String content) throws Exception {
        ...
        KeyPair keyPair = RSAUtil.generateRSAKeyPair(512);

        PublicKey publicKey = keyPair.getPublic();
        PrivateKey privateKey = keyPair.getPrivate();
        LogUtil.msg(publicKey.toString());
        LogUtil.msg(privateKey.toString());

        LogUtil.msg("公钥加密——私钥解密");
        LogUtil.msg("明文：\n" + string);
        LogUtil.msg("明文大小：" + string.getBytes().length);
        /***********************  加密    ********************/

        String encodedData = publicEncrypt(string, (RSAPublicKey) publicKey);
        LogUtil.msg("密文：\n" + encodedData);
        LogUtil.msg("密文大小：" + encodedData.getBytes().length);
