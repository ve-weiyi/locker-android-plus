package com.ve.lib.common.view.widget.view
...
class CircleImageView(context: Context, attrs: AttributeSet) : AppCompatImageView(context, attrs) {
    ...
    init {
        ...
        mBorderWidth = ta.getDimensionPixelSize(R.styleable.CircleImageView_borderWidth, dip2px(
            DEFAULT_BORDER_WIDTH
        ))
        mRectRoundRadius =
            ta.getDimensionPixelSize(R.styleable.CircleImageView_rectRoundRadius, dip2px(
                DEFAULT_RECT_ROUND_RADIUS
            ))
        //val a=ta.getDimensionPixelSize(maxWidth)
        ta.recycle()
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (rawBitmap != null && mType != TYPE_NONE) {
            val viewWidth = Math.min(width,maxWidth)
            val viewHeight = Math.min(height,maxHeight)
