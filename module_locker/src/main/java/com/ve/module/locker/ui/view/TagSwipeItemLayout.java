 ...
 */
public class TagSwipeItemLayout extends ViewGroup {
    enum Mode {
    ...
    public TagSwipeItemLayout(Context context) {
        this(context, null);
    }

    public TagSwipeItemLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.TagSwipeItemLayout);
        mSideVisibility=a.getBoolean(R.styleable.TagSwipeItemLayout_sideVisibility,false);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            if (i == 0) {
                ...
                childLeft = pl + mainLp.leftMargin;
                childTop = pt + mainLp.topMargin;
                childRight = getWidth() - (pr + mainLp.rightMargin);
                childBottom = getHeight() - (mainLp.bottomMargin + pb);
...
//    }
    ...
    class ScrollRunnable implements Runnable {
        ...
        void startScroll(int startX, int endX) {
            if (startX != endX) {
                ...
                ViewCompat.postOnAnimation(TagSwipeItemLayout.this, this);
            }
        }
    ...
    public static class OnSwipeItemTouchListener implements RecyclerView.OnItemTouchListener {
        private TagSwipeItemLayout mCaptureItem;
        ...
        public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent ev) {
            ...
            switch (action) {
                case MotionEvent.ACTION_DOWN: {
                    ...
                    TagSwipeItemLayout pointItem = null;
                    //首先知道ev针对的是哪个item
                    View pointView = findTopChildUnder(rv, (int) x, (int) y);
                    if (pointView == null || !(pointView instanceof TagSwipeItemLayout)) {
                        //可能是head view或bottom view
                        pointOther = true;
                    } else
                        pointItem = (TagSwipeItemLayout) pointView;
    ...
    public static void closeAllItems(RecyclerView recyclerView) {
        for (int i = 0; i < recyclerView.getChildCount(); i++) {
            ...
            if (child instanceof TagSwipeItemLayout) {
                TagSwipeItemLayout swipeItemLayout = (TagSwipeItemLayout) child;
