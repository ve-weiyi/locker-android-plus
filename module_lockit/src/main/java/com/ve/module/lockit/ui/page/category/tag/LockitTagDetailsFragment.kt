 ...
 */
class LockitTagDetailsFragment :
    BaseVmFragment<LockitFragmentDetailsTagBinding, LockitClassifyViewModel>() {

    companion object {

        const val FRAGMENT_TYPE_KEY: String = "lockitDetailsTagFragment.type"
        const val FRAGMENT_DATA_KEY: String = "lockitDetailsTagFragment.key"
    }

    override fun attachViewBinding(): LockitFragmentDetailsTagBinding {
        return LockitFragmentDetailsTagBinding.inflate(layoutInflater)
    }
    ...
    override fun initObserver() {
        ...
        mViewModel.tagAddMsg.observe(this){
            showMsg(it)
            EventBus.getDefault().post(RefreshDataEvent(LockitTagListFragment::class.java.name))
            activity?.finish()
        }
